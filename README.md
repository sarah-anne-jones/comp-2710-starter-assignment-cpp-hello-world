# :wave: Getting to know Class Tools 

## 🤓 Assignment overview and learning outcomes 

The goal of this assignment is to give you a brief introduction to the tools that you will be using to complete homework assignments in this course. We’ll also provide you with materials for further learning and tutorials with pictures that walk you through everything. 🚀

## :octocat: Using Jenkins

Jenkins is the tool that we will be using to automate compiling and running our code. In order to even access the Jenkins Server, you will need to first log onto the Auburn VPN. You will find information on how to do this in Module 1 resources. 

For instructions on how to use Jenkins, please see the pdf on using Jenkins that you will find here in this repo. 

## :octocat: Github API token 

We will need an api key in order to allow jenkins to access our github repo (since they will be set to private). Please see the tutorial on creating a github api token.

### Repositories

A repository is where your project work happens--think of it as your project folder. It contains all of your project’s files and revision history.  You can work within a repository alone or invite others to collaborate with you on those files. Each group should have it's own repository. It doesn't matter which group member initiates the repository, but that group member will need to invite the other one. Only ONE group member should accept the invitation.

## :desktop_computer: CPP

All of the code that you upload to your github repos should be in a similar format (so that it will work more easily with jenkins). You should have your code files (.cpp) and you should have a makefile that is used to build that code. Take a look at the examples here. Experiment with some of the code and run the job in Jenkins to see what happens. 

Modify the code as described in main.cpp and run a job on jenkins that will compile and run the file. Your program should produce some output in the jenkins console.

## :writing_hand: Github Codespaces

You are welcome to use any editor that you like. I encourage you to take a look at the github codespaces editor. This will allow you to code "directly" from the repo and is already configured to commit the code back to the repo for you.


## 📝 Optional next steps 

* Run the code directly in the terminal using Github Codespaces.
* Use different commands in the makefile to trigger different events.
* Go to the link below to learn more about how branching works in github. It's a very fun and interactive way to learn.

## 📚  Resources 
* https://learngitbranching.js.org/
